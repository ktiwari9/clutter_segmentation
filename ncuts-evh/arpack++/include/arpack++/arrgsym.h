...
class ARrcSymGenEig:
  ...
  virtual public ARrcSymStdEig<ARFLOAT> {
	  ...
	  typedef ARrcGenEig<ARFLOAT, ARFLOAT> genSuperT;
...
inline void ARrcSymGenEig<ARFLOAT>::ChangeInvertMode(char InvertModep)
{
  ...
  switch (InvertMode) {
  case 'S':
	  genSuperT::mode    = 3;    // Shift and invert mode.
    break;
  case 'B':
	  genSuperT::mode    = 4;    // Buckling mode.
    break;
  case 'C':
	  genSuperT::mode    = 5;    // Cayley mode.
    break;
  }
  genSuperT::iparam[7] = genSuperT::mode;
  genSuperT::Restart();
...
inline void ARrcSymGenEig<ARFLOAT>::ChangeShift(ARFLOAT sigmap)
{
	...
	genSuperT::sigmaR    = sigmap;
	genSuperT::sigmaI    = 0.0;

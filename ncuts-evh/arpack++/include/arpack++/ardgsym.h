...
class ARluSymGenEig:
  public virtual ARSymGenEig<ARFLOAT, ARdsSymPencil<ARFLOAT>,
	  ...
	  typedef ARSymGenEig<ARFLOAT, ARdsSymPencil<ARFLOAT>, ARdsSymPencil<ARFLOAT> > genSuperT;
...
Copy(const ARluSymGenEig<ARFLOAT>& other)
{
  ...
  Pencil = other.Pencil;
  genSuperT::objOP  = &Pencil;
  genSuperT::objB   = &Pencil;
  genSuperT::objA   = &Pencil;
...
inline void ARluSymGenEig<ARFLOAT>::ChangeShift(ARFLOAT sigmap)
{
	...
	genSuperT::objOP->FactorAsB(sigmap);
...

{
  ...
  DefineParameters(A.ncols(), nevp, &Pencil,
                   ...
                   &ARdsSymPencil<ARFLOAT>::MultBv, whichp,
                   ncvp, tolp, maxitp, residp, ishiftp);
  genSuperT::InvertMode = genSuperT::CheckInvertMode(InvertModep);
  switch (genSuperT::InvertMode) {
  case 'B':
    ChangeMultBx(&Pencil, &ARdsSymPencil<ARFLOAT>::MultAv);
  case 'S':
    ChangeShift(sigmap);

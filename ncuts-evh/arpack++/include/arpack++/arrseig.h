...
class ARrcStdEig {
 ...
 // b.3) Functions that check user defined parameters.
  ...
  virtual const char* CheckWhich(const char* whichp);
 ...
 // c.11) Functions that use STL vector class.
  ...
  std::vector<ARTYPE>* StlArnoldiBasisVectors();
  // Returns a copy of the Arnoldi basis in a single STL vector.

  std::vector<ARTYPE>* StlArnoldiBasisVector(int i);
  // Returns the i-th Arnoldi basis vector in a STL vector.

  std::vector<ARTYPE>* StlEigenvectors(bool ischur = false);
  ...
  std::vector<ARTYPE>* StlSchurVectors();
  // Returns a copy of the Schur vectors in a single STL vector.

  std::vector<ARTYPE>* StlSchurVector(int i);
  // Returns the i-th Schur vector in a STL vector.

  std::vector<ARTYPE>* StlResidualVector();
...
template<class ARFLOAT, class ARTYPE>
const char* ARrcStdEig<ARFLOAT, ARTYPE>::CheckWhich(const char* whichp)
{
...
template<class ARFLOAT, class ARTYPE>
inline std::vector<ARTYPE>* ARrcStdEig<ARFLOAT, ARTYPE>::StlArnoldiBasisVectors()
{

  // Returning the Arnoldi basis in a single STL vector.

	std::vector<ARTYPE>* StlBasis;

  if (!BasisOK) {
    nconv = FindArnoldiBasis();
  }
  try {
    StlBasis = new std::vector<ARTYPE>(&V[1], &V[n*ncv+1]);
  }
...
template<class ARFLOAT, class ARTYPE>
inline std::vector<ARTYPE>* ARrcStdEig<ARFLOAT,ARTYPE>::StlArnoldiBasisVector(int i)
{
  ...
  try {
    StlBasis = new std::vector<ARTYPE>(&V[i*n+1], &V[(i+1)*n+1]);
  }
  catch (ArpackError) { return NULL; }
  return StlBasis;
...
template<class ARFLOAT, class ARTYPE>
inline std::vector<ARTYPE>* ARrcStdEig<ARFLOAT,ARTYPE>::StlEigenvectors(bool ischur)
{

  // Returning the eigenvector in a single STL vector.

	std::vector<ARTYPE>* StlEigVec;
  ARTYPE*         VecPtr;

  try { StlEigVec = new std::vector<ARTYPE>(ValSize()*n); }
  catch (ArpackError) { return NULL; }
  VecPtr = StlEigVec->data();//->begin();
...
template<class ARFLOAT, class ARTYPE>
inline std::vector<ARTYPE>* ARrcStdEig<ARFLOAT, ARTYPE>::StlSchurVectors()
{

	std::vector<ARTYPE>* StlSchurVec;

  if (!SchurOK) {
    nconv = FindSchurVectors();
  }
  try {
    StlSchurVec = new std::vector<ARTYPE>(&V[1], &V[nev*n+1]);
  }
  catch (ArpackError) { return NULL; }
  return StlSchurVec;
...
template<class ARFLOAT, class ARTYPE>
inline std::vector<ARTYPE>* ARrcStdEig<ARFLOAT, ARTYPE>::StlSchurVector(int i)
{
  ...
  try {
    StlSchurVec = new std::vector<ARTYPE>(&V[i*n+1], &V[(i+1)*n+1]);
  }
  catch (ArpackError) { return NULL; }
  return StlSchurVec;
...
template<class ARFLOAT, class ARTYPE>
inline std::vector<ARTYPE>* ARrcStdEig<ARFLOAT, ARTYPE>::StlResidualVector()
{

  // Returning the residual vector in a STL vector.

	std::vector<ARTYPE>* StlResid;

  if (!newRes) {
    throw ArpackError(ArpackError::RESID_NOT_OK, "StlResidualVector");
  }
  try {
    StlResid = new std::vector<ARTYPE>(resid, &resid[n]);
  }
  catch (ArpackError) { return NULL; }
  return StlResid;

...
int ARStdEig<ARFLOAT, ARTYPE, ARFOP>::FindArnoldiBasis()
{
  ...
  while (!superT::BasisOK) {
    ...
    if ((superT::ido == -1) || (superT::ido == 1)) {

      // Performing Matrix vector multiplication: y <- OP*x.

      (objOP->*MultOPx)(&superT::workd[superT::ipntr[1]],&superT::workd[superT::ipntr[2]]);
